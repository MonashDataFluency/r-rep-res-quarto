...
Now, let's open RStudio. If you are using your local RStudio, make sure you use IDE version RStudio v2022.07.1 or later and that it is running [Quarto version 1.2 or above](https://quarto.org/docs/download/prerelease.html). If you need to check that, for RStudio, choose `Help` and `About RStudio`. For the Quarto version checking, type in `packageVersion("quarto")` on the console. 
